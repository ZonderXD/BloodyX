...
async def join(ctx):
    ...
    if voice is not None:
        ...
        await ctx.send(embed = discord.Embed(descriprion = f"{ctx.author.mention}, я успешно подключился к голосовому каналу!"))

@bot.command(pass_context = True, aliases = ['p', 'P','pl','Pl','Play','play'])
...
async def leave(ctx):
    ...
    if voice and voice.is_connected():
        ...
        await ctx.send(embed = discord.Embed(descriprion = f"{ctx.author.mention}, я успешно отключился от голосового канала!"))
    else:
        await ctx.send(embed = discord.Embed(descriprion = f"{ctx.author.mention}, простите, но я не в голосе."))
...
async def giveaway( ctx, seconds: int, *, text ):
    ...
    message = await ctx.send(embed = discord.Embed(
        ...
        text = '𝐂𝐲𝐛𝐞𝐫𝐬𝐩𝐚𝐜𝐞 ツ#9691 © | Все права защищены',
...
async def botinfo(ctx):
    ...
    embed.add_field(name=f'**Меня создал:**', value="`𝐂𝐲𝐛𝐞𝐫𝐬𝐩𝐚𝐜𝐞 ツ#9691`(<@716724192065749075>)", inline=False)  # Создает строку
