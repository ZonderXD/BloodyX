...
async def help(ctx):
    ...
    embed1.add_field(name='💎 Базовые:', value='**``.user [@user]`` - Узнать информацию о пользователе 🎭\n ``.server`` - Узнать информацию о сервере 🧿\n `.bot` - Информация о боте 🤖\n`.avatar [@user]` - Аватар пользователя 🖼\n `.suggest (text)` - Предложить идею ✉\n `.wiki (text)` - Википедия 📖**', inline = False)
    embed2 = discord.Embed(title = '⚙ Навигация по командам:\n ❗ Обязательные параметры: `()`\n ❓ Необязательные параметры: `[]`\n\n', color=0x6fdb9e )
    embed2.add_field(name='🎶 Роблокс музыка:', value='**`.music` - Коды для музыки 💨**', inline = False)
    embed3 = discord.Embed(title = '⚙ Навигация по командам:\n ❗ Обязательные параметры: `()`\n ❓ Необязательные параметры: `[]`\n\n', color=0x6fdb9e )
    embed3.add_field(name='🥥 Скрипты:', value='**`.scripts` - Скрипты для игр 🍕**', inline = False)
    embed4 = discord.Embed(title = '⚙ Навигация по командам:\n ❗ Обязательные параметры: `()`\n ❓ Необязательные параметры: `[]`\n\n', color=0x6fdb9e )
    embed4.add_field(name='🎉 Весёлости:', value='**``.ran_color`` - Рандомный цвет в формате HEX 🩸\n ``.coin`` - Бросить монетку 🌈\n ``.math (2*2/2+2-2)`` - Решить пример :infinity:\n `.8ball (question)` - Волшебный шар 🔮\n `.password (10 10)` - Рандомный пароль 🎩\n `.meme` - Рандомный мем 🤣**', inline = False)
    embed5 = discord.Embed(title = '⚙ Навигация по командам:\n ❗ Обязательные параметры: `()`\n ❓ Необязательные параметры: `[]`\n\n', color=0x6fdb9e )
    embed5.add_field(name='💋 Некос:', value='**`.hug (@user)` - Обнять 😜\n `.slap (@user)` - Ударить 😡\n `.ran_avatar` - Рандом. аватар 🤯\n `.kill [@user]` - Убить 🔪\n `.dog` - Собака :dog:\n `.goose` - Гусь :duck:\n `.cat` - Кот 🐱**', inline = False)
